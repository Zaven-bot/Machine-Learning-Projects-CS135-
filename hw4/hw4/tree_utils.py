...
class InternalDecisionNode(object):
    ...
    def predict(self, x_TF):
        ...
        leftBoolMask = x_TF[:, self.feat_id] <= self.thresh_val
        ...
        leftPred = self.left_child.predict(x_TF[leftBoolMask])
        rightPred = self.right_child.predict(x_TF[rightBoolMask])
        ...
        yhat_T[leftBoolMask] = leftPred
        yhat_T[rightBoolMask] = rightPred
        return yhat_T
