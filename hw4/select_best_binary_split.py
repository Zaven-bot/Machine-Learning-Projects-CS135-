...
def select_best_binary_split(x_NF, y_N, MIN_SAMPLES_LEAF=1):
    ...
    for f in range(F):
        ...
        for v, threshold in enumerate(possib_xthresh_V):
            # Split the data at threshold
            leftBoolMask = x_NF[:, f] <= threshold
            rightBoolMask = ~leftBoolMask

            # Get indices of left and right children
            leftChildren = np.where(leftBoolMask)[0]
            rightChildren = np.where(rightBoolMask)[0]

            # Calculate avg. pred. for left and right children
            leftYhat = np.average(y_N[leftChildren])
            rightYhat = np.average(y_N[rightChildren])

            # Store arrays with values
            left_yhat_V[v] = leftYhat
            right_yhat_V[v] = rightYhat

            # Calculate squared error
            leftCost = np.sum(np.square(y_N[leftChildren] - leftYhat))
            rightCost = np.sum(np.square(y_N[rightChildren] - rightYhat))

            # Store arrays with costs
            left_cost_V[v] = leftCost
            right_cost_V[v] = rightCost
        ...
        # Get index of minimum total cost among all threshold
        chosen_v_id = np.argmin(total_cost_V)
        # Update threshold and cost according to minimum total cost
