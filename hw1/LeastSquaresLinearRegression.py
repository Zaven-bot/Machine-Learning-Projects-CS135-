    ...
    * self.b : float
    ...
    def fit(self, x_NF, y_N):
        ...
        * self.b (scalar real bias, if desired)
        ...
        N, F = x_NF.shape
        x_NF1 = np.zeros((N, F + 1))
        x_NF1[:,:F] = x_NF
        x_NF1[:, F] = 1
        
        xTx_F1F1 = np.dot(x_NF1.transpose(), x_NF1)
        xy_F1F1 = np.dot(x_NF1.transpose(), y_N)
        theta_F11 = np.linalg.solve(xTx_F1F1, xy_F1F1)
        
        # other elements are weights
        self.w_F = theta_F11[:-1]
        # last elements are the bias terms
        self.b = theta_F11[-1]
