...
def calc_mean_binary_cross_entropy_from_probas(ytrue_N, yproba1_N):
    ...
    mean_bce = np.mean(bce_prob)
    return mean_bce  # TODO fix me
...
def calc_mean_binary_cross_entropy_from_scores(ytrue_N, scores_N):
    ...
    scores_and_zeros_N2 = np.column_stack(([np.zeros(N), flipped_scores_N]))  # TODO fix me: [0, flipped_scores_N]. Hint: stack two arrays of shape (N,1)

    bce_values = []

    for row in scores_and_zeros_N2:
        bce_val = scipy_logsumexp(row) / np.log(2)
    ...
    bce_score = np.sum(bce_values) / N # TODO fix me
